...
atlas_path = os.path.join(settings.resources_path, '10um_Atlas');
"""Default path to atlas infomration.
"""

default_annotation_file = os.path.join(atlas_path, 'ABA_10um_annotation.tif');
...
default_reference_file = os.path.join(atlas_path, 'ABA_10um_reference.tif');
"""Default volumetric annotated image file.
...
default_distance_to_surface_file = os.path.join(atlas_path, 'ABA_10um_distance_to_surface.tif');
"""Default volumetric annotated image file.
...
def label_points(points, annotation_file = None, invalid = 0, key = 'order', level = None):
  ...
  print(points)
  print(points_int)
  print(points_int.sum())
  for d in tqdm.tqdm(range(n_dim)):
    if d == 0:
