...
class Main_Window(QWidget):
    ...
    def resize_pictures(self, _voxel_size_signal_x=5,_voxel_size_signal_y=2,_voxel_size_signal_z=2,_voxel_size_auto_x = 5,_voxel_size_auto_y=2,_voxel_size_auto_z=2):
    ...
    def detect_cells(self,_number_of_free_cpus=4,_n_sds_above_mean_thresh=10, _trained_model="",_soma_diameter=16\
            , _xy_cell_size=6, _z_cell_size=6, _gaussian_filter=0.2, _orientation = "asl",_batch_size = 256):        
        ...
        if self.my_working_directory != "":
            ...
            if os.path.exists(self.my_working_directory +"/" + self.channel_chosen + "_voxel_size_signal"):
                ...
                final_string = basic_string + signal_string + background_string + voxel_sizes_string + threshold_string +\
                        trained_model_string + cpu_string + soma_diameter_string + xy_cell_size_string + z_cell_size_string+\
                        gaussian_filter_string + batch_size_string + orientation_string + output_string 
    ...
    def train_network(self, _yaml_files="",_trained_model = "",_continue_training=True, _test_fraction=0.1, _learning_rate=0.0001, _batch_size=32, _epochs=1):
        ...
        if not os.path.exists(self.my_working_directory + "/" + self.channel_chosen + "_output_training"):
            ...
            if _trained_model != "":
                run_training(output_dir=Path(_output_directory),trained_model = _trained_model,yaml_file=[Path(_yaml_files)],install_path=install_path,learning_rate=_learning_rate\
                        ,continue_training=_continue_training,test_fraction=_test_fraction,batch_size=_batch_size,save_progress=True,epochs=_epochs)
            else:
                if os.path.exists(_output_directory):
                    run_training(output_dir=Path(_output_directory),yaml_file=[Path(_yaml_files)],install_path=install_path,learning_rate=_learning_rate\
                        ,continue_training=_continue_training,test_fraction=_test_fraction,batch_size=_batch_size,save_progress=True,epochs=_epochs)
                else:
                    alert = QMessageBox()
    ...
    def preprocess_layout(self):
        ...
        start_preprocess_button.pressed.connect(lambda: self.resize_pictures(_voxel_size_signal_x = float(voxel_size_signal_x.text()), _voxel_size_signal_y = float(voxel_size_signal_y.text())\
                ,_voxel_size_signal_z = float(voxel_size_signal_z.text()),_voxel_size_auto_x = float(voxel_size_auto_x.text()) ,_voxel_size_auto_y = float(voxel_size_auto_y.text()) , _voxel_size_auto_z = float(voxel_size_auto_z.text())))
    ...
    def cd_layout(self):
        ...
        def save_config(save_path):
            if not os.path.exists(save_path):    
                print(save_path)
                resample_variable_list = [n_cpus.text(),n_sds_above_mean.text(),soma_diameter.text(),\
                    xy_cell_size.text(),z_cell_size.text(),gaussian_filter.text(),\
                    trained_model.text(),orientation.text()]
                pd_df = pd.DataFrame([resample_variable_list], index = [1], columns = ["Number of CPUs", "Number SDS above mean"\
                        , "Soma Diameter","XY Plane Cell Size","Z Plane Cell Size","Gaussian Filter","Trained Model","Orientation"])
       ...
       ### Connection of Widgets with cell_detection functions 
        ...
        start_cell_detection_button.clicked.connect(lambda: self.detect_cells(_number_of_free_cpus=int(n_cpus.text()),_n_sds_above_mean_thresh=int(n_sds_above_mean.text())\
                , _trained_model=str(trained_model.text()),_soma_diameter=int(soma_diameter.text()), _xy_cell_size=int(xy_cell_size.text())\
                , _z_cell_size=int(z_cell_size.text()), _gaussian_filter=float(gaussian_filter.text()), _orientation = str(orientation.text())))
    ...
    def training_layout(self):  
        ...
        def save_config(save_path):
            if not os.path.exists(save_path):    
                print(save_path)
                resample_variable_list = [continue_training.isChecked(),test_fraction.text(),learning_rate.text(),\
                    batch_size.text(),epochs.text()]
                pd_df = pd.DataFrame([resample_variable_list], index = [1], columns = ["Continue training", "Test fraction"\
        ...
        train_network_button.clicked.connect(lambda: self.train_network(_yaml_files=str(yaml_files.text()),_trained_model =str(trained_model.text())\
                ,_continue_training=bool(continue_training.isChecked()), _test_fraction=float(test_fraction.text()), _learning_rate=float(learning_rate.text())\
                , _batch_size=int(batch_size.text()), _epochs=int(epochs.text())))
