...
"""
def create_result_frame(df, tracked_list):
...
"""
def analyse_csv(df: utils.pd.DataFrame,reference_df: utils.pd.DataFrame, tracked_levels: list, choice: str, my_working_directory: str) -> utils.pd.DataFrame:
    ...
    resultframe = create_result_frame(reference_df, tracked_levels)
...
def embeded_ontology(choice, my_working_directory):
    ...
    samplename = utils.os.path.basename(my_working_directory)
    new_df = analyse_csv(df,reference_df, tracked_levels, choice, my_working_directory)
